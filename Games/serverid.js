        ...
        if (enableFriendservers) {
            ...
            if (friendServerContainer) {
                ...
                allButtons.forEach(function(button) {
                    if (button.textContent && button.textContent.toLowerCase().includes('share')) {
                        button.remove();
                    }
                });
            }
...
function createAndAddCopyJoinLinkButton(server, serverId, serverIDTextElement = null) {
    ...
    allElements.forEach(function(element) {
        if (element.textContent && element.textContent.toLowerCase().includes('share') && 
            ...
            element.remove();
        } else if (element.hasAttribute('aria-label') && element.getAttribute('aria-label').toLowerCase().includes('share')) {
            element.remove();
        } else if (element.hasAttribute('title') && element.getAttribute('title').toLowerCase().includes('share')) {
            element.remove();
        }
    });
...
function addVipServerInviteButton(server, vipServerId) {
    ...
    if (existingButtonContainer) {
            ...
            vipStatusCache[vipServerId] = 'success'; 
        }
        return; 
    }

    if (cachedStatus === 'no-link' || cachedStatus === 'error') {
