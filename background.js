...
chrome.runtime.onMessage.addListener(async (message, sender, sendResponse) => {
    switch (message.action) {
        ...
        case 'disableUserSniper':
            ...
            break;
        case 'enableCountryCodeRule':
          console.log("enableCountryCodeRule");
           await updateDeclarativeNetRequestRuleWithCookie('ruleset_3');
            chrome.declarativeNetRequest.updateEnabledRulesets({
                enableRulesetIds: ['ruleset_3'],
            });
            break;
        case 'disableCountryCodeRule':
           console.log("disableCountryCodeRule");
             chrome.declarativeNetRequest.updateEnabledRulesets({
...
async function applyUpdate() {
    ...
    if (latestVersion && compareVersions(latestVersion, currentVersion) > 0) {
        ...
        chrome.tabs.create({
            url: `https://github.com/NotValra/RoValra/releases/tag/v${latestVersion}`
        });
    } else {
